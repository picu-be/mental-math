...
const getTerms = (): Term[] => {
  ...
  for (let i = 0; i < maxEquations; i++) {
    const denominator = Math.floor(Math.random() * 49) + 1; // Generate a denominator between 1 and 49
    const factor = Math.floor(Math.random() * 49) + 1; // Generate a factor (quotient) between 1 and 49
    const numerator = denominator * factor; // Calculate the numerator
